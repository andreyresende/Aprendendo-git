...
void Opcao(int escolheu){//Funcao utilizada pela funcao Menu, no momento so funciona para instrucoes. Colocada antes no codigo para evitar Warnings no terminal, ja que se ela for chamada por uma funcao antes de ser declarada ele reclama com "Implicit Declaration".
    ...
    switch (escolheu){
        ...
        case 4 :
            CLEAR;
            printf("Ola, Bem vindo ao River Raid, C Version.\n");
            printf("Comandos: w(move o personagem para cima), s(move o personagem para baixo) e d(atira)\n");
            printf("Voce eh uma nave (+), inimigos(X) virao pela direita, bem como combustivel(F). Encostar em um inimigo finaliza o jogo, encostar em um (F) aumenta o seu combustivel em 40 unidades.\n");
